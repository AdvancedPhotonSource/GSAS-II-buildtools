#=======================================================================
# This is being developed to run on BHT21 to compile and package GSAS-II
# Packaging is not possible yet because constructor is not available
#    for Apple Silicon
# there are also at this time (12/22) compatibility issues for conda-build
#   which does not help
#=======================================================================
#which gfortran
...
mkdir -p /tmp/bin
ln -s /opt/homebrew/bin/gcc-12 /tmp/bin/gcc  # create a link to hb gcc
export PATH=/tmp/bin:/opt/homebrew/bin:$PATH # put HB tools 1st in path
...
then
    ...
    echo conda create -n $env python=3.10 wxpython=4.1 numpy=1.23 scipy matplotlib pyopengl pillow h5py imageio conda requests scons -y
         conda create -n $env python=3.10 wxpython=4.1 numpy=1.23 scipy matplotlib pyopengl pillow h5py imageio conda requests scons -y
    echo conda create -n bld python conda-build -y
         conda create -n bld python conda-build -y
    # echo conda create -n cons python constructor -d
    #      conda create -n cons python constructor -d
fi
...
then
  ...
  which gfortran
  which gcc
  export FLIBloc="/opt/homebrew/opt/gcc/lib/gcc/current/"
  rm -rf $WORKSPACE/GSASII/AllBinaries
  ...
  echo "Built with MacOSarm.sh on BHT21 from G2 version $SVN_REVISION on $NODE_LABELS" >> ../AllBinaries/$newdir/Build.notes.txt
...
then
  ...
  $svn ci -m"Build with MacOSarm.sh on BHT21" $WORKSPACE/Binaries
fi
#
...
then
	...
	echo remember to copy staged files to /home/joule/SVN/subversion/pyGSAS/downloads/ using svnpgsas@s11bmbcda.xray.aps.anl.gov
