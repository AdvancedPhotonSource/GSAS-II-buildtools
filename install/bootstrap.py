...
version = "$Id: bootstrap.py 5459 2022-12-30 04:54:57Z toby $"
...
skipProxy = False          # if False then the script creates a prompt asking for proxy info
                           #--> True: -nonet or -noproxy or -binary=
showWXerror = False        # if True, errors are shown in wxPython windows (used on Windows only)
                           #--> True: -noinstall or -binary= (both on windows)
help = False               # if True, a help message is shown
                           #--> True: -help
allBinaries = False        # if True, causes all binaries to be installed
                           #--> True: -allbinaries or -server

npVersion=None          # when set, gives a specific numpy version to use (such as 1.18)
                           # for selecting a set of binaries to use
                           #--> Set by: -binary=

pyVersion=None             # when set, gives a specific Python version to use (such as 3.7)
                           # for selecting a set of binaries to use
                           #--> Set by: -binary=
...
for a in sys.argv[1:]:
    if 'noinstall' in a.lower():
        ...
        if len(vers) > 1:
            pyVersion = vers[1]
            print('Selecting Python binary',pyVersion)
        print('Selecting numpy binary',npVersion)
        skipInstallChecks = True
        if sys.platform.startswith('win'): showWXerror = True
        skipProxy = True
    else:
...
if help:
  ...
  bootstrap.py options:
    ...
    --binary=npver          specifies a specific numpy/python version to use in selecting 
    --binary=npver,pyver    a binary directory to use ; turns on --noinstall and --noproxy
           (example --binary=1.18 or --binary=1.18,3.7)
           turns on --noinstall and --noproxy
''')
    sys.exit()
...
def svncleanup(spath,svn=None):
    if not svn: svn = whichsvn()
    if not svn: return
    svntmp = os.path.join(spath,'.svn','tmp')
    if os.path.exists(os.path.join(spath,'.svn')) and not os.path.exists(svntmp):
        print('missing subversion tmp directory, fixing')
        os.makedirs(svntmp,exist_ok=True)
    if os.path.exists(os.path.join(spath,'.svn')):
        cmd = [svn, 'cleanup', spath]
        s = subprocess.Popen(cmd,stderr=subprocess.PIPE)
        out,err = MakeByte2str(s.communicate())
        if err:
            print('subversion cleanup returned an error:')
            if out: print(out)
            if err: print(err)
...
def svnChecksumPatch(svn,fpath,verstr):
    ...
    unclear reasons. 
    '''
    svntmp = os.path.join(fpath,'.svn','tmp')
    if not os.path.exists(svntmp):
        print('missing subversion tmp directory')
    ...
    out,err = MakeByte2str(s.communicate())
    print(svntmp, os.path.exists(svntmp))
    #if err: print('error=',err)
    try:
        import GSASIIpath
        print('import of GSASIIpath completed')
    except Exception as err:
        msg = 'Failed with import of GSASIIpath in svnChecksumPatch. This is unexpected.'
...
if not skipDownloadSteps:
    ...
    if '.xor.' in str(res):
        ...
        cmd = [svn, 'switch','--relocate','https://subversion.xor.aps.anl.gov/pyGSAS',
               'https://subversion.xray.aps.anl.gov/pyGSAS',path2GSAS2]
        if proxycmds: cmd += proxycmds
        p = subprocess.Popen(cmd,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        res,err = p.communicate()
        if err:
    ...
    # patch: switch GSAS-II location if switched to 2frame version (removed August 2017)
        ...
        if err:
            ...
            print(err)
            print(res)
    elif 'not a working' not in str(res):
        print('Previous svn installation found, prepare for update with a cleanup')
        svncleanup(path2GSAS2)

    print('\n'+75*'*')
    print('Now preparing to install or update GSAS-II')
    tryagain = True
    err = False
    firstPass = 0
    while(tryagain):
...
if skipDownloadSteps:
    ...
    binaryVersion = GSASIIpath.GetBinaryPrefix(pyVersion)+'_n'+npVersion
    print('Load binaries from '+binaryVersion)
    ...
    print('Load binaries matching current python/numpy')
