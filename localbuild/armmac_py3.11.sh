# on BHT21, using homebrew (hb) to access gfortran
#
...
svn=`which svn`
miniforge=https://github.com/conda-forge/miniforge/releases/download/23.11.0-0/Mambaforge-23.11.0-0-MacOSX-arm64.sh
FLIBloc=/opt/homebrew/opt/gcc/lib/gcc/current/
...
if [ "$install" = "True" ]
then
	conda create -y -n $env python=3.11 wxpython numpy scipy matplotlib pyopengl pillow h5py imageio conda requests scons #  anaconda-client constructor conda-build sphinx sphinx-rtd-theme git
fi
...
then
  ...
  echo "Using gfortran, info follows" >> $WORKSPACE/GSASII/AllBinaries/$newdir/Build.notes.txt
...
if [ "$upload" = "True" ]
then
    if [ ! -d $WORKSPACE/GSASII/AllBinaries/$newdir ]; then
