# BHT21 use homebrew (hb) to access gfortran
#
...
if [ "$dryrun" = "True" ]
then
    conda create --dry-run -n test python=3.12 wxpython numpy scipy matplotlib pyopengl pillow h5py imageio conda requests scons meson
    exit
fi

if [ "$install" = "True" ]
then
	conda create -y -n $env python=3.12 wxpython numpy scipy matplotlib pyopengl pillow h5py imageio conda requests scons meson #  anaconda-client constructor conda-build sphinx sphinx-rtd-theme git
fi
